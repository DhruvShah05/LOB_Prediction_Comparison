...
**Change 1.1.3 [NICE] — Optional second asset (ETHUSDT).**
*Decision: not required.* Reviewer 2's single-asset point is a scoping criticism and is answered without new data by (a) scoping every claim to "BTCUSDT perpetual futures on Binance", (b) using all 12 days instead of 3 and reporting per-regime / per-day robustness (Change 5.4), (c) the FI-2010 ↔ crypto transfer experiment (Change 5.7), and (d) listing "single asset, single exchange" in Limitations as a stated boundary.

If time allows, a second asset closes the point outright. There is no published ETHUSDT dataset in the same 250 ms / 10-level Binance-perpetual format, so it must be recorded:
- New script `scripts/record_binance_depth.py`: subscribe to Binance USDT-M Futures websocket streams `btcusdt@depth10@250ms` and `ethusdt@depth10@250ms` simultaneously; write each message (exchange timestamp, local timestamp, 10 bids, 10 asks) to daily parquet files; auto-reconnect on disconnect and log every reconnect. Run on a small VPS for ≥ 12 days. Start it early — it runs in the background while everything else is built.
- Output must match the schema `prepare_crypto.py` expects, so `--symbol ETHUSDT` is a drop-in.
- Recording BTC alongside ETH gives a fresh BTC window over the *same dates*, which is cleaner for a two-asset comparison than pairing new ETH data with the old Kaggle BTC file.
- Fallback (cross-exchange, not like-for-like): the same Kaggle author's Coinbase dataset (BTC/ETH/ADA, 1 s snapshots, 15 levels, distance-from-mid format) at https://www.kaggle.com/datasets/martinsn/high-frequency-crypto-limit-order-book-data — would need reconstruction `price = mid × (1 + distance)` and should be presented as a separate cross-exchange check.

Priority: **backtest (5.6) > regime split (5.4) > second asset.** If the second asset isn't done by write-up, drop it and keep the scoping language.
...
**Change 1.3.3 [R2] — Split ratios and regime coverage (this replaces the need for a second asset).**
Keep 70/15/15 chronological for crypto but on all 12 days (~8.4 / 1.8 / 1.8 days). Additionally support `split_by_day: {train:[...], val:[...], test:[...]}` so test can be set to specific days. Add a **rolling-origin evaluation**: e.g. train days 1–6 / val 7 / test 8; train 1–7 / val 8 / test 9; … through test day 12. Report mean ± std across folds as a second robustness axis alongside seeds. This directly shows whether results hold across different market days/regimes within the one asset.
...
- One published temporal baseline besides DeepLOB: **DeepLOB-Attention** (Zhang & Zohren 2021, "Multi-Horizon Forecasting for Limit Order Books"). *Decision: use this, not TransLOB.* It reuses the DeepLOB encoder from 2.1 with an attention seq2seq decoder, gives multi-horizon output from one model (feeds the horizon sweep and backtest directly), and shares the FI-2010 protocol with DeepLOB so published numbers are comparable. Mention TransLOB (Wallbridge 2020) in related work only.
...
### 5.4 New `experiments/regime_split.py` [R2] — primary answer to "one asset"

On the crypto test period compute rolling realized volatility (e.g. 5-minute std of 250 ms mid returns) and rolling spread / depth. Bucket test rows into low/mid/high tertiles for each. Recompute all metrics per bucket from saved predictions. Also report per-calendar-day metrics and the rolling-origin folds from Change 1.3.3. Figures: Macro-F1 and MCC per regime per model; per-day metrics as a line plot. Discussion must state explicitly that this establishes within-asset robustness across regimes, and that cross-asset generalization is outside the study's scope.
...
data:
  ...
  symbol: BTCUSDT                     # crypto only; ETHUSDT only if recorded (1.1.3)
...
|---|---|
| Title / Abstract | Scope every claim to "BTCUSDT perpetual futures on Binance" — never "cryptocurrency markets" as a generalization. Reframe: "isolating why LOB models degrade on crypto — feature set, labeling rule, or market" or "above-chance classification ≠ profitability". State the headline numbers from the matched (raw40_eng, smoothed) setting. |
...
| §IX Limitations | Only things *not* fixed. State "single asset (BTCUSDT), single exchange (Binance)" as a deliberate scope boundary, pointing to the regime/rolling-origin robustness and transfer results as within-scope evidence. Also: no market-impact model, no order-flow/trade features, no multi-exchange latency model. |
...
5. **Train** the grid × 5 seeds. Minimum grid: 2 markets × 2 feature sets × 2 label rules × 7 models (logistic, RF, XGB, DeepLOB, windowed Transformer, LevelTransformer, DeepLOB-Attention) × 5 seeds = 280 runs. Budget accordingly; trees are fast, neural runs dominate. ETH (if recorded) adds 70 runs and is done last.
...
- [ ] Regime split and rolling-origin folds reported for crypto; all claims scoped to BTCUSDT/Binance. (Second asset optional — include only if recorded and run.)
